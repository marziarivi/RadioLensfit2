...
#pragma omp parallel
    ...
    if (argc < 6)
    {
        ...
        cout << "usage: RadioLensfit.x <filename u-coord> <filename v-coord> <min flux> <shear1> <shear2> " << endl;
        exit(EXIT_FAILURE);
    }
    ...
    double freq_start_hz = 1.4e+9;        // Start Frequency, in Hz
    ...
    //double maxB = read_uv_txt(argv[1], argv[2], num_coords, uu_metres, vv_metres, &len);
    if (rank==0)
    {
        cout << "max baseline: " << maxB << " , " << len << endl;
    ...
    // Allocate Raw Data Visibilities -----------------------------------------------------------------------------------------------------------------------
