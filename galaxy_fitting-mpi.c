...
#ifdef FACET
void source_extraction(int rank, unsigned int my_start_index, complexd *facet_vis, double *facet_sigma2, double l0, double m0, double flux, double mu, double e1, double e2, likelihood_params *par, complexd *visSkyMod, complexd *visData, complexd *visGal, 
                       double *sigma2_vis, unsigned int nchannels, unsigned long int num_coords, double *uu_metres, double *vv_metres, double *ww_metres, double len)
#else
void source_extraction(int rank, unsigned int my_start_index, double l0, double m0, double flux, double mu, double e1, double e2, likelihood_params *par, complexd *visSkyMod, complexd *visData, complexd *visGal, 
                       unsigned int nchannels, unsigned long int num_coords, double *uu_metres, double *vv_metres, double *ww_metres)
#endif
{
#ifdef FACET
   ...
   // temporary galaxy visibilities for one channel at a time (cannot be used by more threads with OpenMP!)
   complexd *visGal_ch = (complexd *) malloc(num_coords*sizeof(complexd)); 
...
//#endif
   for (unsigned int ch = 0; ch < nchannels; ch++)
   {
     ...
     // get small round source at the current position and flux for this channel
     data_galaxy_visibilities((par->spec)[ch_ind], (par->wavenumbers)[ch_ind], par->band_factor, par->acc_time, e1, e2, mu,
                              flux, l0, m0, num_coords, uu_metres, vv_metres, ww_metres, visGal_ch);
     unsigned long int k = 0;
     for (unsigned long int i = ch_vis; i<ch_vis+num_coords; i++)
     {
        // remove it from the sky model
        visSkyMod[i].real -= visGal_ch[k].real;
        visSkyMod[i].imag -= visGal_ch[k].imag;
        ...
        // add round source back to the sky model
        visSkyMod[i].real += visGal_ch[k].real;
        visSkyMod[i].imag += visGal_ch[k].imag;
     ...
     // gridding visibilities and store them at the corresponding proc-th section of facet_vis and sigma2
...
void source_fitting(int rank, likelihood_params *par, double *mes_e1, double *mes_e2, double *var_e1, double *var_e2, double *oneDimvar, double *maxL)
{
